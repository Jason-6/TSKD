...
def main(args, large_args):
    ...
    large_args = compute_subsampling_factor(large_args)
    ...
    large_args.vocab = train_set.vocab
    large_args.vocab_sub1 = train_set.vocab_sub1
    large_args.vocab_sub2 = train_set.vocab_sub2
    large_args.input_dim = train_set.input_dim
    ...
    if args.resume:
        ...
        if args.mbr_training:
            ...
            large_args.save_path = mkdir_join(args.model_save_dir, '_'.join(
                os.path.basename(args.train_set).split('.')[:-1]), dir_name)
    ...
    large_model = Speech2Text(large_args, args.save_path, train_set.idx2token[0])
    large_model.eval()
    average_checkpoints(large_model, large_args.recog_model[0], n_average=large_args.recog_n_average)

    model.large_model = large_model
    ...
    for ep in range(resume_epoch, args.n_epochs):
        train_one_epoch(model, large_model, train_set, dev_set, eval_sets,
        ...
        if reporter.n_epochs + 1 < args.eval_start_epoch:
            ...
            if True or scheduler.is_topk or is_transformer:
                # Save model
                if args.local_rank == 0:
                    scheduler.save_checkpoint(
                        model, args.save_path, amp=amp,
...
def train_one_epoch(model, large_model, train_set, dev_set, eval_sets,
                    scheduler, reporter, logger, args, amp, scaler,
                    tasks, teacher, teacher_lm):
...
if __name__ == '__main__':
    small_model_config = "../../../examples/aishell/s5/conf/asr/kd/layer/ctc_kd/conformer_kernel15_clamp10_hie_subsample8_las_ln_layer_1.yaml"
    small_args = parse_args_train(sys.argv[1:], small_model_config)
    small_args.distributed = small_args.n_gpus > 1 and small_args.local_world_size > 1

    large_model_config = "../../../examples/aishell/s5/conf/asr/kd/layer/ctc_kd/conformer_kernel15_clamp10_hie_subsample8_las_ln_layer_12.yaml"
    large_args = parse_args_train(sys.argv[1:], large_model_config)
    large_args.distributed = large_args.n_gpus > 1 and large_args.local_world_size > 1
    large_args.recog_model = [
        "../../../examples/aishell/s5/kd/layer/ctc_kd/data/results/aishell1/asr/train_sp/conv2Lconformer256dmodel1024dff10L4Hkernel15_layer_norm_clamp10_max_pool2_noam_lr5.0_bs18000_ls0.1_warmup25000_accum8_ctc1.0_27FM2_100TM2/model.epoch-60"]
    large_args.recog_n_average = 10
    main(small_args, large_args)
