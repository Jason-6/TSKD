...
class DecoderBase(ModelBase):
    ...
    def _plot_attention(self, save_path=None, n_cols=2):
        ...
        for k, aw in aws_dict.items():
            ...
            for h in range(n_heads):
                ...
                ax.grid(False)
                self.addInformation(ax, aw)
                ax.set_xlabel("Input (head%d)" % h)
                ax.set_ylabel("Output (head%d)" % h)
                # ax.xaxis.set_major_locator(MaxNLocator(integer=True))
                # ax.yaxis.set_major_locator(MaxNLocator(integer=True))
                # ax.set_yt/media/mgl/OS/data/dataset/eval_set_sp.tsvicks(np.linspace(0, ylens[-1] - 1, ylens[-1]))
    ...
    def addInformation(self, ax, aw):
        _, _, h, w = aw.shape
        for i in (range(h)):
            ax.plot([0, w - 1], [i, i], "r--")
        # texts = list("海南拐两八洞速度正常联系广州幺三四五再见幺三四五再见拐两八洞速度正常")
        # texts.append("<EOS>")
        # ax.set_yticks(list(range(h)), texts)
        env_dist = os.environ
        file_name = env_dist.get("file_name")
        print("=" * 30, aw.shape, "=" * 30)
        if file_name is not None:
            np.save(file_name, aw)
