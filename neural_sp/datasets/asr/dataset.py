...
class CustomDataset(Dataset):
    ...
    def __init__(self, corpus, tsv_path, tsv_path_sub1, tsv_path_sub2,
        ...
        if is_test and len(df["speaker"].str.contains('hk')) != 0:
            df = df[df["speaker"].str.contains('hk')]
        for i in range(1, 3):
        ...
        self.wav_input = df['feat_path'].iloc[0].split('.')[-1] in ['wav']
        self._input_dim = kaldiio.load_mat(df['feat_path'].iloc[0]).shape[-1]
                        ...
                        for j in range(1, i + 1):
                            setattr(self, 'df_sub' + str(j),
                                    getattr(self, 'df_sub' + str(j)).drop(
                                        getattr(self, 'df_sub' + str(j)).index.difference(df.index)))
    ...
    def __len__(self):
        if not self.is_hk and not self.is_test:
            return len(self.df[~self.df["speaker"].str.contains('hk')])
        else:
            return len(self.df[self.df["speaker"].str.contains('hk')])
