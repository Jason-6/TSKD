...
class CustomBatchSampler(sampler):
    ...
    def __init__(self, dataset, distributed, batch_size, batch_size_type,
                 ...
                 longform_max_n_frames=0, seed=1, resume_epoch=0, start_hk_epoch=200):
        """Custom BatchSampler.
        ...
        self.start_hk_epoch = start_hk_epoch
        ...
        if shuffle_bucket and self.start_hk_epoch > self.current_epoch:
            self.indices_buckets = shuffle_bucketing(self.df[~self.df["speaker"].str.contains('hk')],
                                                     self.batch_size,
                                                     batch_size_type, self.dynamic_batching,
                                                     seed=seed + resume_epoch,
                                                     num_replicas=self.num_replicas)
        elif shuffle_bucket and self.start_hk_epoch <= self.current_epoch:
            self.indices_buckets = shuffle_bucketing(self.df[self.df["speaker"].str.contains('hk')], self.batch_size,
                                                     batch_size_type, self.dynamic_batching,
                                                     seed=seed + resume_epoch,
                                                     num_replicas=self.num_replicas)
            dataset.is_hk = True
    ...
    def reset(self, batch_size=None, batch_size_type=None, epoch=0):
        ...
        if self.shuffle_bucket and self.start_hk_epoch > self.current_epoch:
            self.indices_buckets = shuffle_bucketing(self.df[~self.df["speaker"].str.contains('hk')],
                                                     batch_size, batch_size_type, self.dynamic_batching,
                                                     seed=self.seed + epoch,
                                                     num_replicas=self.num_replicas)
        elif self.shuffle_bucket and self.start_hk_epoch <= self.current_epoch:
            self.indices_buckets = shuffle_bucketing(self.df[self.df["speaker"].str.contains('hk')],
                                                     batch_size, batch_size_type, self.dynamic_batching,
                                                     seed=self.seed + epoch,
                                                     num_replicas=self.num_replicas)
            self.dataset.is_hk = True
