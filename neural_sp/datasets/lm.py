...
class Dataset(object):
    ...
    def __init__(self, tsv_path, batch_size, bptt, distributed=False,
                 ...
                 shuffle=False, backward=False, serialize=False, corpus='', start_hk_epoch=200):
        ...
        if (self.epoch == self.start_hk_epoch):
            self.concat_ids = self.concat_utterances(self.df[self.df["speaker"].str.contains('hk')])
        else:
            self.concat_ids = self.concat_utterances(self.df[~self.df["speaker"].str.contains('hk')])
