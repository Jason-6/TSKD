...
# ASR configuration
#########################
conf=conf/asr/paper/acoustic/conformer_kernel15_clamp10_hie_subsample8_las_ln_ctc_9.yaml
conf2=
asr_init=
external_lm=
...
feature_data=/vol/nfs/mgl/datasets/data_mfcc
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ] && [ ! -e ${feature_data}/.done_stage_1 ]; then
    ...
    echo "================================="
    for x in train dev test; do
        steps/make_mfcc.sh --nj 32 --cmd "$train_cmd" --write_utt2num_frames true \
            ${feature_data}/${x} ${feature_data}/log/make_mfcc/${x} ${feature_data}/mfcc || exit 1;
        utils/fix_data_dir.sh ${feature_data}/${x}
    done

    speed_perturb_3way_mfcc.sh ${feature_data} train ${train_set}
